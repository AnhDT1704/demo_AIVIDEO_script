...
class GeminiFrameAnalyzer:
    ...
    def analyze_scene_clip_with_gemini(self, clip_path: str, scene_id: int) -> Dict[str, Any]:
        ...
        try:
            ...
            import time
            max_wait_time = 300  # 60 seconds timeout
        ...
        for i, scene in enumerate(scenes_with_clips):
            ...
            if scene.get('clip_path') and os.path.exists(scene['clip_path']):
                ...
                try:
                    ...
                    # ✅ FIXED: Preserve existing speaker and transcription fields, only add visual info
                    enriched_scene = scene.copy()
                    enriched_scene['visual_description'] = visual_analysis.get('visual_description', '')
                    enriched_scene['scene_type'] = visual_analysis.get('scene_type', 'video_analyzed')
                    enriched_scene['visual_analysis_error'] = visual_analysis.get('error', None)

                except Exception as e:
                    # st.write(f"❌ **DEBUG**: Exception during Scene {scene['scene_id']} analysis: {str(e)}")
                    enriched_scene = scene.copy()
                    # Keep speaker_transcript and unique_speakers from scene
                    enriched_scene['visual_description'] = ''
                    enriched_scene['scene_type'] = 'unknown'
                    enriched_scene['visual_analysis_error'] = f'Exception: {str(e)}'
                ...
                enriched_scene['visual_description'] = ''
                enriched_scene['scene_type'] = 'unknown'
                enriched_scene['visual_analysis_error'] = scene.get('extraction_error', 'No clip extracted')
        ...
        for scene in scenes_with_clips:
            ...
            if clip_path and os.path.exists(clip_path):
                try:
                    ...
                    pass  # Silent cleanup - don't show warnings for cleanup failures
